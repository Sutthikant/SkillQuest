...
def generate_skilltree(skill_name_and_description: str):
    ...
    prompt = (f"""
                    ...
                    - Be as deep as necessary (4 to 5 levels of nodes or more if needed).
                    ...
                    - The parents node must be a bigger idea of the children node.
                    - Include a `prerequisites` field for each node. This field should list the names (or IDs) of other nodes that must be completed before the current one can be accessed. If the node has no prerequisites, return an empty list, the deepest node should not have any prerequisites. 
                    - ** The children nodes must not have the parents node as a prerequisites. **
                    ...
                    - You should occationally increase some pratice quest.
                    ...
                    - Return the result in **pure JSON** format (no additional text or explanation) **only pure json**.
                    - Be structured and ready for integration into a UI.

                    The main skill and description is: {skill_name_and_description}

                    Please return the skill tree in the following JSON format **note that please strigly return in this format, no other text or string at all!!!**:
